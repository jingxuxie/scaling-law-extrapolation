...
A decision consequence is especially severe. Define
$C_k(\tau)=\min\{M:\Risk_k(M)-E\le\tau\}$. Integral bounds on $\phi_\alpha$ imply
\begin{align}
 C_1(\tau)&\ge \left(\frac{B}{\alpha\tau}\right)^{1/\alpha}-1,\\
 C_0(\tau)&\le \left\lceil\left(\frac{A}{\beta\tau}\right)^{1/\beta}\right\rceil.
 \label{eq:compute-target}
\end{align}
Their ratio is unbounded as $\tau\downarrow0$. Hence a below-noise component can have little effect on the next measured loss while changing the resource budget required for a demanding target by orders of magnitude.
...
$\eta=\sigma/(2\sqrt m)$. For any estimator $\widehat\ell_\tau$ of
$\ell_k(\tau)=\log C_k(\tau)$,
\begin{equation}
 \max_{k\in\{0,1\}}\E_k
 |\widehat\ell_\tau-\ell_k(\tau)|
 \ge \frac{3}{16}\left|\log\frac{C_1(\tau)}{C_0(\tau)}\right|.
 \label{eq:compute-minimax}
\end{equation}
For all sufficiently small $\tau$, Eqs.~\eqref{eq:compute-target} imply that the right-hand side grows at least as
\begin{equation}
