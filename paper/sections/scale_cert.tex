...
An empty set rejects the declared class at the chosen tolerance. An unbounded upper LP is an honest abstention. For a loss target $\tau$, success is certified when $\widetilde U_\delta(M_\star)\le\tau$, failure when $\widetilde L_\delta(M_\star)>\tau$, and otherwise remains unidentified. Bounded structural mismatch is included by expanding each pilot band by a declared radius $\rho_i$.
...
\begin{align}
 \underline C_\delta(\tau)&:=\inf\{M:\widetilde L_\delta(M)\le\tau\},\\
 \overline C_\delta(\tau)&:=\inf\{M:\widetilde U_\delta(M)\le\tau\}.
 \label{eq:compute-certificate}
\end{align}
Then, for every threshold $\tau$ simultaneously,
\begin{equation}
 \underline C_\delta(\tau)
 \le C_{\Risk}(\tau)
 \le \overline C_\delta(\tau).
 \label{eq:compute-coverage}
\end{equation}
Either endpoint may be infinite. Thus an unbounded upper resource requirement records that the pilot data cannot guarantee the target loss under the declared class.
\end{corollary}
